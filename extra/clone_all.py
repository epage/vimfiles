...
def prepare_repo(root, repo):
    ...
    if hook.exists():
        ...
        if managed_hook_path.exists() and "File generated by prek: https://github.com/j178/prek" not in managed_hook_path.read_text():
            subprocess.run(["prek", "install"], cwd=repo_root, capture_output=True, check=True)

    p = subprocess.run(["git", "-C", repo_root, "rev-parse", "--abbrev-ref", "HEAD"], encoding="utf-8", capture_output=True, check=True)
    main_branch = p.stdout.strip()
    for branch in repo.get("branches", []):
        subprocess.run(["git", "-C", repo_root, "fetch", "origin", branch], encoding="utf-8", capture_output=True, check=True)
        subprocess.run(["git", "-C", repo_root, "checkout", branch], encoding="utf-8", capture_output=True, check=True)
    subprocess.run(["git", "-C", repo_root, "checkout", main_branch], encoding="utf-8", capture_output=True, check=True)
